...
describe ('GET /candidates', done => {
  ...
  it ('should return array of Objects', async () => {
    ...
    const res = await request(app).get('/candidates').expect(200)
  ...
  it ('should call Candidate.find() with status parameters when `status` args is passed in', async () => {
    const candidateFindStub = sinon.stub(Candidate, 'find').resolves([{name: 'Tim', status:'pending'}])
    
    const res = await request(app).get('/candidates?status=pending').expect(200)
...
describe ('GET /candidates/:candidateId', async () => {
  ...
  it ('should call findById with the correct Parameters', async () => {
    ...
    expect(candidateFindStub.getCall(0).args).to.be.an('array').that.does.include('5abf3dcf1d567955609d2bd4')
    candidateFindStub.restore()
  })

  it ('should return a Candidate', async () => {
    const candidateFindStub = sinon.stub(Candidate, 'findById').resolves(expected)
    const res = await request(app).get('/candidates/5abf3dcf1d567955609d2bd4').expect(200)
